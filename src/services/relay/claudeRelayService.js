              ...
              if (line.startsWith('data:')) {
                ...
                try {
                  ...
                  if (data.type === 'message_start' && data.message && data.message.usage) {
                    ...
                    const messageInputDetails = messageUsage.input_tokens_details || {}

                    // message_start包含input tokens、cache tokens和模型信息
                    currentUsageData.input_tokens = messageUsage.input_tokens ?? 0
                    currentUsageData.cache_creation_input_tokens =
                      messageUsage.cache_creation_input_tokens ??
                      messageUsage.cacheCreateTokens ??
                      messageInputDetails.cache_creation_input_tokens ??
                      messageInputDetails.cacheCreateTokens ??
                      0
                    currentUsageData.cache_read_input_tokens =
                      messageUsage.cache_read_input_tokens ??
                      messageUsage.cacheReadTokens ??
                      messageUsage.cacheReadInputTokens ??
                      messageInputDetails.cache_read_input_tokens ??
                      messageInputDetails.cacheReadTokens ??
                      messageInputDetails.cacheReadInputTokens ??
                      0
                  ...
                  // message_delta可能包含最终完整的usage信息
                  if (data.type === 'message_delta' && (data.usage || data.delta?.usage)) {
                    const deltaUsage = data.usage || data.delta.usage
                    const deltaInputDetails = deltaUsage.input_tokens_details || {}

                    if (deltaUsage.output_tokens !== undefined) {
                      currentUsageData.output_tokens = deltaUsage.output_tokens || 0
                    }

                    if (deltaUsage.input_tokens !== undefined) {
                      currentUsageData.input_tokens = deltaUsage.input_tokens || 0
                    }

                    if (deltaUsage.cache_creation_input_tokens !== undefined) {
                      currentUsageData.cache_creation_input_tokens =
                        deltaUsage.cache_creation_input_tokens ?? 0
                    } else if (deltaUsage.cacheCreateTokens !== undefined) {
                      currentUsageData.cache_creation_input_tokens =
                        deltaUsage.cacheCreateTokens ?? 0
                    } else if (deltaInputDetails.cache_creation_input_tokens !== undefined) {
                      currentUsageData.cache_creation_input_tokens =
                        deltaInputDetails.cache_creation_input_tokens ?? 0
                    } else if (deltaInputDetails.cacheCreateTokens !== undefined) {
                      currentUsageData.cache_creation_input_tokens =
                        deltaInputDetails.cacheCreateTokens ?? 0
                    }

                    if (deltaUsage.cache_read_input_tokens !== undefined) {
                      currentUsageData.cache_read_input_tokens =
                        deltaUsage.cache_read_input_tokens ?? 0
                    } else if (deltaUsage.cacheReadTokens !== undefined) {
                      currentUsageData.cache_read_input_tokens = deltaUsage.cacheReadTokens ?? 0
                    } else if (deltaUsage.cacheReadInputTokens !== undefined) {
                      currentUsageData.cache_read_input_tokens =
                        deltaUsage.cacheReadInputTokens ?? 0
                    } else if (deltaInputDetails.cache_read_input_tokens !== undefined) {
                      currentUsageData.cache_read_input_tokens =
                        deltaInputDetails.cache_read_input_tokens ?? 0
                    } else if (deltaInputDetails.cacheReadTokens !== undefined) {
                      currentUsageData.cache_read_input_tokens =
                        deltaInputDetails.cacheReadTokens ?? 0
                    } else if (deltaInputDetails.cacheReadInputTokens !== undefined) {
                      currentUsageData.cache_read_input_tokens =
                        deltaInputDetails.cacheReadInputTokens ?? 0
                    }

                    if (
                      deltaUsage.cache_creation &&
                      typeof deltaUsage.cache_creation === 'object'
                    ) {
                      currentUsageData.cache_creation = {
                        ephemeral_5m_input_tokens:
                          deltaUsage.cache_creation.ephemeral_5m_input_tokens || 0,
                        ephemeral_1h_input_tokens:
                          deltaUsage.cache_creation.ephemeral_1h_input_tokens || 0
                      }
                    }
